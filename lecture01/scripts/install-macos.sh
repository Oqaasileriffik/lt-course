...
for PKG in subversion git pkgconfig autoconf automake gawk xz perl5 p5-file-homedir p5-ipc-run p5-app-cpanminus p5-yaml-libyaml p5-json p5-json-xs timeout python314 py314-regex py314-json5 py314-levenshtein py314-yaml py314-click py314-lxml py314-termcolor gsed gmake icu gh realpath
do
	echo "... installing $PKG"
...
GREP=`egrep 'export PYTHONPATH' ~/.profile ~/.zprofile 2>/dev/null | grep python3.14/site-packages`
if [[ -z "$GREP" ]]; then
	echo 'export PYTHONPATH="$PYTHONPATH:/usr/local/lib/python3.14/site-packages"' >> ~/.profile
	echo 'export PYTHONPATH="$PYTHONPATH:/usr/local/lib/python3.14/site-packages"' >> ~/.bashrc
	echo 'export PYTHONPATH="$PYTHONPATH:/usr/local/lib/python3.14/site-packages"' >> ~/.zprofile
fi
